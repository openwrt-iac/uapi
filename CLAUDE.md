...
return {
    ...
    id_for_create: function(body) {...return null;}, // optional; override server-generated ULID when the uci section name has semantic meaning (e.g. network.interfaces for proto=wireguard, where netifd uses the section name as the kernel netdev name and IFNAMSIZ caps it at 15 chars)
...
**ULID exception for kernel-bound section names.** Some uci sections have their `.name` consumed by another subsystem in a way that constrains the format. `proto=wireguard` interfaces are the only case in v2.x: netifd's `wireguard.sh` proto handler uses the section name verbatim as the kernel netdev name, and Linux IFNAMSIZ caps that at 15 chars (a 28-char ULID is rejected silently, and the tunnel never comes up). For these resources, the create path honours an optional `name` field in the request body; absent that, the server emits a shorter `wg_<11-char>` id (14 chars) via `ids.new_id("wg", 11)`. The mechanism is the per-resource `id_for_create(body)` hook on the resource module (see contract above); other resources still get the 28-char ULID. Generalise the hook only when a second case appears, not before.
