...
- **CLI:** `uapi-token create --name <label> --scope <s> [--scope <s>...] [--expires-in <N>[smhd]] [--allowed-cidr <CIDR>...] [--force]`, plus `list` / `show` / `revoke` / `scopes [--json]`. Scopes validated against the known tree, `--force` bypasses for forward-compat with unknown future endpoints. `--expires-in` and `--allowed-cidr` are v2 additions for token expiry and source-IP scoping. `scopes` (2.3.0) enumerates the known scope tree for external consumers (LuCI frontend, fleet tooling).
